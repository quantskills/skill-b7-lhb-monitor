...
| 匹配席位标签 | `seat_tags.match_seat` | 营业部 → 北向/机构/游资/量化/**营业部**/普通 + 席位俗称（确定性规则 + 种子库 + 营业部兜底，覆盖率≈99%） |
...
六类：**北向 / 机构 / 游资 / 量化 / 营业部 / 普通**（优先级：北向>机构>种子库(游资·量化)>营业部>普通）。
- 北向 / 机构：确定性关键词（`沪深股通专用` / `机构专用`），零歧义。
- 游资本尊精确映射 **`seat_yyb_map.json`**（**最高优先级**，营业部全称完全匹配）：由爬取的「游资席位标签集锦」
  经 `build_seat_map.py` 转成，**317 营业部 → 游资本尊/帮派**（章盟主、作手新一、消闲派、佛山系、成都系…），
  含 `alias`(本尊)/`group`(标签类型)/`confidence`(A/B/C 置信度)/`last_seen`。原始爬取文档存 `references/seat_data/`。
  **重爬后**把新 CSV 放进 `references/seat_data/` 再跑 `python scripts/build_seat_map.py` 即可刷新。
- 子串种子库 **`seat_library.json`**（兜底，~55 席：中信溧阳路、宁波桑田路、欢乐海岸…；外资量化：高盛/摩根/中金…），
  每条含 `match`/`category`/`tag`/`group`(帮派)/`tier`/`alias`/`note`。
- 匹配优先级：北向 > 机构 > 外部覆盖 > **爬取精确映射** > 子串种子库 > 营业部兜底 > 普通。
  **维护即改 JSON，不动代码**；`seat_tags.groups()` 给帮派分布。
- **营业部**：种子库未命中但含「营业部/分公司」的具名券商席位——龙虎榜能进买卖前五即显著资金，
  归"营业部"而非"普通"，避免游资盘被漏统计。实测覆盖率≈99%（普通仅剩极少异常名）。
- **游资盘净买 `hotmoney_net` = 游资 + 营业部**（非机构/非北向/非量化的活跃资金）；`known_seat_cnt`=知名游资数，`desk_seat_cnt`=游资盘席位数。
- 外部覆盖：环境变量 `B7_SEAT_OVERRIDE` 指向 JSON `[{"match","category","tag","note"}]`，优先于内置库——把高频出现的"营业部"逐步标注为具体游资即可越用越准。
