...
- 游资 / 量化：独立数据文件 **`seat_library.json`**（按 帮派/级别 组织，~55 席：中信溧阳路、宁波桑田路、
  华泰益田路荣超、东财拉萨系、欢乐海岸、成都系…；外资量化通道：高盛/摩根/中金/中信上海分公司…）。
  每条含 `match`(营业部子串)/`category`/`tag`(俗称)/`group`(帮派)/`tier`(一线·二线)/`alias`(游资本尊，默认空待填)/`note`。
