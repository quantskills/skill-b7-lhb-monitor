...
> 收盘后自动抓取 A 股龙虎榜，用**席位标签库**匹配营业部（北向/机构/游资/量化/营业部/普通，覆盖率≈99%），
...
| 席位库 | `seat_yyb_map.json`(爬取317营业部→章盟主/作手新一/消闲派…本尊+置信度) + `seat_library.json`(种子兜底) | 重爬后把 CSV 放进 `references/seat_data/` 跑 `build_seat_map.py` 刷新；或 `B7_SEAT_OVERRIDE` 临时覆盖 |
...
## 席位标签库（核心资产）
- 北向 / 机构：确定性关键词，零歧义。
- 游资 / 量化：独立数据文件 `scripts/seat_library.json`（按帮派/级别组织，~55 席，含 alias 本尊槽位）（中信溧阳路、宁波桑田路、
  东财拉萨系、欢乐海岸、成都系…）。
